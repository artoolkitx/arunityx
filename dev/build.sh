...
PLUGINS_BASE="${ARUNITYX_HOME}/Packages/artoolkitX-Unity/Runtime/Plugins"
...
locate_artoolkitx() {
    if [ ! -f "${OURDIR}/artoolkitx/LICENSE.txt" ] && [ -z "${ARTOOLKITX_HOME}" ]; then
    ...
    if [  -f "${OURDIR}/artoolkitx/LICENSE.txt" ] && [ -z "${ARTOOLKITX_HOME}" ]; then
        ARTOOLKITX_HOME="${OURDIR}/artoolkitx"
    fi
}
