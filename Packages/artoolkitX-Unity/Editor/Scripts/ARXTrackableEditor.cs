			...
			case ARXTrackable.TrackableType.Square:
                {
					...
					// Look for patterns both inside project and package.
					List<string> PatternFilenames = new List<string>();
					string projectPatternsFolderPath = Path.Join(Application.dataPath, "Patterns");
					if (Directory.Exists(projectPatternsFolderPath))
                    {
						PatternFilenames.AddRange(Directory.GetFiles(projectPatternsFolderPath).Where(s => !s.EndsWith(".meta")).ToList());
					}
					PatternFilenames.AddRange(Directory.GetFiles("Packages/org.artoolkitx.artoolkitx-unity/Editor/Patterns").Where(s => !s.EndsWith(".meta")).ToList());
